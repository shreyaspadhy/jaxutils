...
# New functions implemented are `RandomCropWithPad`, and `Normalize`.
...
class ResizeSmall:
    ...
    def __call__(self, features: Features) -> Features:
        ...
        ratio = tf.cast(self.smaller_size, tf.float32) / tf.cast(
...
class InceptionCrop:
    ...
    def __call__(self, features: Features) -> Features:
        ...
        crop.set_shape(
            [None, None, image.shape[-1]]
        )  # pytype: disable=attribute-error  # allow-recursive-types
        if self.resize_size:
            crop = Resize([self.resize_size, self.resize_size])({"image": crop})[
                "image"
            ]
...
class DecodeJpegAndInceptionCrop:
    ...
    def __call__(self, features: Features) -> Features:
        ...
        if self.resize_size:
            image = Resize([self.resize_size, self.resize_size])({"image": image})[
                "image"
            ]
...
@dataclasses.dataclass
class RandomCropWithPad:
    """Performs a random crop after padding (parity with torchvision.RandomCrop).
    Attributes:
      crop_size: Either an integer H, where H is both the height and width of the
        random crop, or a list or tuple [H, W] of integers, where H and W are
        height and width of the random crop respectively.
        padding: Optional padding on border of each image.
      key: Key of the data to be processed.
      key_result: Key under which to store the result (same as `key` if None).
      rng_key: Key of the random number used for
        `tf.image.stateless_sample_distorted_bounding_box`.
    ...
    def __call__(self, features: Features) -> Features:
        ...
        if self.padding is not None:
            paddings = _maybe_repeat(self.padding, 2)
            pad_size = [x + y for x, y in zip(crop_size, paddings)]
            image = tf.image.pad_to_bounding_box(image, *paddings, *pad_size)
        cropped_image = tf.image.stateless_random_crop(
            image, [crop_size[0], crop_size[1], image.shape[-1]], seed=rng
        )  # pytype: disable=attribute-error  # allow-recursive-types
        features[self.key_result or self.key] = cropped_image
...
@dataclasses.dataclass
class Normalize:
    """Normalize an image by mean and std. deviation.

    Attributes:
      mean: A tuple of values of size n_channels.
      std: A tuple of values of size n_channels.
      key: Key of the data to be processed.
      key_result: Key under which to store the result (same as `key` if None).
    """
    ...
    def __call__(self, features: Features) -> Features:
        image = features[self.key]
        mean = tf.constant(self.mean, tf.float32)
...
class Onehot:
    ...
    def __call__(self, features: Features) -> Features:
        ...
        if (
            labels.shape.rank > 0 and self.multi
        ):  # pytype: disable=attribute-error  # allow-recursive-types
            x = tf.scatter_nd(
                labels[:, None], tf.ones(tf.shape(labels)[0]), (self.depth,)
            )
